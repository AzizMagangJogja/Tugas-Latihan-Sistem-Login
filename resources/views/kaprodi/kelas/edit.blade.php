                ...
                <div class="flex flex-col flex-1 min-h-screen">
                    ...
                    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200 flex flex-col">
                        <div class="container px-6 py-8 mx-auto flex-grow">
                            <h3 class="text-3xl font-bold text-gray-700">Silakan Edit Kelas</h3>
                            ...
                            <div class="flex flex-col mt-auto">
                                <div class="py-2 -my-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                                    <div class="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow-lg rounded-lg bg-white">
                                        <form action="{{ route('kaprodi.kelas.update', $kelas->id) }}" method="POST" class="bg-white shadow-lg rounded-lg p-8">
                                            @csrf
                                            @method('PUT')

                                            <div class="mb-4">
                                                <label for="name" class="block text-sm font-medium">Nama Kelas</label>
                                                <input type="text" name="name" id="name" value="{{ old('name', $kelas->name) }}" class="block w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:ring-blue-300" required>
                                            </div>

                                            <div class="mb-4">
                                                <label for="jumlah" class="block text-sm font-medium">Jumlah Mahasiswa</label>
                                                <input type="number" name="jumlah" id="jumlah" value="{{ old('jumlah', $kelas->jumlah) }}" class="block w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:ring-blue-300" required>
                                            </div>
                                            
                                            <div class="mb-4">
                                                <label for="dosen_id" class="block text-sm font-medium">Dosen Wali</label>
                                                <select name="dosen_id" id="dosen_id" class="block w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:ring-blue-300">
                                                    <option value="">Tidak Ada Dosen Wali</option>
                                                    @foreach ($dosens as $dosen)
                                                        <option value="{{ $dosen->id }}" 
                                                            {{ old('dosen_id', $kelas->dosen ? $kelas->dosen->id : '') == $dosen->id ? 'selected' : '' }}>
                                                            {{ $dosen->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
