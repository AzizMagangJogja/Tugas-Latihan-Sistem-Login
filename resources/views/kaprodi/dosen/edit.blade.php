        ...
        <div>       
            <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-gray-200">
                ...
                <div class="flex flex-col flex-1 min-h-screen">
                    ...
                    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200 flex flex-col">
                        <div class="container px-6 py-8 mx-auto flex-grow">
                            <h3 class="text-3xl font-bold text-gray-700">Silakan Edit Dosen</h3>
                    
                            <div class="container mx-auto p-6">
                                @if ($errors->any())
                                    <div class="bg-red-500 text-white p-4 rounded-lg inline-flex items-center mb-auto">
                                        <ul>
                            ...
                            <div class="flex flex-col mt-auto">
                                <div class="py-2 -my-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                                    <div class="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow-lg rounded-lg bg-white">
                                        <form action="{{ route('kaprodi.dosen.update', $dosen->id) }}" method="POST" class="bg-white shadow-lg rounded-lg p-8">
                                            @csrf
                                            @method('PUT')

                                            <div class="mb-4">
                                                <label for="nip" class="block text-sm font-medium text-gray-700">NIP</label>
                                                <input type="text" name="nip" id="nip" value="{{ old('nip', $dosen->nip) }}" class="block w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:ring-blue-300" required>
                                            </div>
                        
                                            <div class="mb-4">
                                                <label for="name" class="block text-sm font-medium text-gray-700">Nama Dosen</label>
                                                <input type="text" name="name" id="name" value="{{ old('name', $dosen->name) }}" class="block w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:ring-blue-300" required>
                                            </div>
                            
                                            <div class="mb-4">
                                                <label for="kode_dosen" class="block text-sm font-medium text-gray-700">Kode Dosen</label>                                                    
                                                <input type="text" name="kode_dosen" id="kode_dosen" value="{{ old('kode_dosen', $dosen->kode_dosen) }}" class="block w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:ring-blue-300" required>
                                            </div>
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <select name="kelas_id" id="kelas_id" class="block w-full p-3 border border-gray-300 rounded-lg focus:ring focus:ring-blue-300 focus:outline-none">
                                                    ...
                                                    @foreach ($kelas as $item)
                                                        <option value="{{ $item->id }}" {{ old('kelas_id', $dosen->kelas_id) == $item->id ? 'selected' : '' }}>
                                                            {{ $item->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
