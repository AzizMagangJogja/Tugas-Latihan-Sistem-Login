                ...
                <div class="flex flex-col flex-1 overflow-hidden">
                    ...
                    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
                        <div class="container px-6 py-8 mx-auto min-h-screen flex-grow">
                            <h3 class="text-3xl font-bold text-gray-700 mb-5 ">Daftar Request Edit Data Mahasiswa</h3>
                                        ...
                                        <table class="min-w-full divide-y divide-gray-200">
                                            ...
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                ...
                                                <tr class="hover:bg-gray-100 transition duration-200">
                                                    <td class="px-6 py-4 text-sm leading-5 text-gray-800 whitespace-no-wrap">{{ $request->mahasiswa->name }}</td>
                                                    <td class="px-6 py-4 text-sm leading-5 text-gray-800 whitespace-no-wrap">{{ $request->keterangan }}</td>
                                                    <td class="px-6 py-4 text-sm leading-5 whitespace-no-wrap">
                                                        <form action="{{ route('dosen.request.approve', $request->id) }}" method="POST" class="inline-block">
                                                            @csrf
                                                            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-700 transition duration-300 ease-in-out">Setujui</button>
                                                        </form>
                                    
                                                        <form action="{{ route('dosen.request.reject', $request->id) }}" method="POST" class="inline-block">
                                                            @csrf
                                                            <button type="submit" class="bg-red-600 text-white px-4 py-2 rounded-lg shadow hover:bg-red-700 transition duration-300 ease-in-out">Tolak</button>
                                                        </form>
                                                    </td>
                                                </tr>
                                                @endforeach
