        ...
        <div>
            <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-gray-200">
                ...
                <div :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform bg-gray-900 lg:translate-x-0 lg:static lg:inset-0">
                    ...
                    <nav class="mt-10">
                        <a class="flex items-center px-6 py-2 mt-4 text-gray-100 bg-gray-900 bg-opacity-25 hover:bg-gray-700" href="{{ route('dosen.request') }}">
                        ...
                        <div class="container px-6 py-8 mx-auto min-h-screen flex-grow">
                            ...
                            <div class="mt-auto">
                                <div class="flex flex-wrap -mx-6">
                                        ...
                                        <div class="flex items-center px-5 py-6 bg-white rounded-md shadow-sm">
                                            ...
                                            <div class="mx-5">
                                                @if ($kelas)
                                                    ...
                                                    <div class="text-gray-500">Mahasiswa {{ $kelas->name }}</div>
                                                @else
                                                    <h4 class="text-2xl font-semibold text-gray-700">{{ $jumlahMahasiswa }}</h4>
                                    ...
                                    <div class="justify-center w-full sm:w-1/2 xl:w-1/3 xl:mt-0">
                                        @if (Auth::check())
                                        @php
                                        ...
                                        @if (!$dosen || !$dosen->kelas_id)
                                            <div class="p-4 text-sm text-red-700 bg-red-100 rounded-lg shadow-sm" role="alert">
                                                <span class="font-medium">Perhatian!</span> Anda bukan dosen wali. Anda tidak diperbolehkan mengakses kelas!
                                            </div>
                                        @endif
                                        @endif
                                    </div>
                            ...
                            <div class="mt-8">
                                <div class="flex flex-wrap space-x-4 mx-6">
                                    @if (Auth::user()->role == 'dosen_wali' && Auth::user()->dosen && Auth::user()->dosen->kelas_id)
                                        <a href="{{ route('dosen.create') }}" class="flex items-center px-4 py-2 text-sm text-white bg-green-500 rounded-lg shadow hover:bg-green-600 transition duration-300 ease-in-out transform hover:scale-105">
                                        ...
                                        <a href="{{ route('dosen.request') }}" class="flex items-center px-4 py-2 text-sm text-white bg-blue-500 rounded-lg shadow hover:bg-blue-600 transition duration-300 ease-in-out transform hover:scale-105">
                                            <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <div class="py-2 -my-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                                    <div class="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow sm:rounded-lg">
                                        <table class="min-w-full">
                                            <thead>
                                                <tr>
                                                    <th class="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase border-b border-gray-200 bg-gray-50">NIM</th>
                                                    <th class="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase border-b border-gray-200 bg-gray-50">NAMA MAHASISWA</th>
                                                    <th class="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase border-b border-gray-200 bg-gray-50">TEMPAT LAHIR</th>
                                                    <th class="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase border-b border-gray-200 bg-gray-50">TANGGAL LAHIR</th>
                                                    <th class="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase border-b border-gray-200 bg-gray-50">KODE REQUEST EDIT</th>
                                            ...
                                            <tbody class="bg-white">
                                                ...
                                                <tr>
                                                    <td class="px-6 py-4 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">{{ $mahasiswa->nim }}</td>
                                                    <td class="px-6 py-4 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">{{ $mahasiswa->name }}</td>
                                                    <td class="px-6 py-4 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">{{ $mahasiswa->tempat_lahir }}</td>
                                                    <td class="px-6 py-4 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">{{ $mahasiswa->tanggal_lahir }}</td>
                                                    <td class="px-6 py-4 text-sm leading-5 text-gray-500 whitespace-no-wrap border-b border-gray-200">{{ $mahasiswa->edit }}</td>
                                                    <td class="px-6 py-4 text-sm leading-5 whitespace-no-wrap border-b border-gray-200">
                                                        ...
                                                        @if ($dosen && $dosen->kelas_id)
                                                            <div class="flex space-x-2">
                                                                <a href="{{ route('dosen.edit', $mahasiswa->id) }}" class="inline-block px-4 py-2 text-sm text-white bg-indigo-600 rounded-lg shadow hover:bg-indigo-700 transition duration-300 ease-in-out transform hover:scale-105">
                                                                    Edit
                                                                </a>
                                                                <form action="{{ route('dosen.destroy', $mahasiswa->id) }}" method="POST" class="inline-block">
                                                                    @csrf
                                                                    @method('DELETE')
                                                        ...
                                                        @else
                                                            <span class="text-gray-500 font-medium">Tidak ada aksi</span>
                                                        @endif
                                                    </td>                                                                                           
                                                </tr>
                                                @endforeach
